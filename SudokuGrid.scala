...
case class SudokuGrid private (
    private val grid: Vector[Vector[SudokuGrid.Cell]]
):
  ...
  override def toString(): String =
    "SudokuGrid:\n" +
      grid.zipWithIndex
        .map((rowVec, i) =>
          val rowStr = rowVec.zipWithIndex
            .map((colVal, j) =>
              if j % 3 == 2 then s"$colVal "
              else colVal.toString()
            )
            .mkString(" ")

          if i % 3 == 2 then s"$rowStr\n"
  ...
  private def boxAt(row: Int, col: Int): Vector[(Cell, Int, Int)] =
    ...
    val boxIndexSet =
      boxRowRange
        .flatMap(row => Vector(row, row, row))
        .zip(boxColRange :++ boxColRange :++ boxColRange)
        .toSet
  ...
  def solved(using visualize: Visualize[SudokuGrid]): Option[SudokuGrid] =
    lowestEntropy match
      ...
      case Some((minRow, minCol)) =>
        grid(minRow)(minCol).possibleValues.iterator
          .map(value =>
            // visualize(this)
  ...
  lazy val lowestEntropy: Option[(Int, Int)] =
    cellsWithIndexes
      .collect { case cell @ (Cell.Unknown(_), _, _) => cell }
  ...
  infix def where(locIsValue: ((Int, Int), Int)): SudokuGrid =
    ...
    newGrid
      ...
      .foreach((cell, rowIndex, colIndex) =>
  ...
  infix def where(locsAndValues: ((Int, Int), Int)*): SudokuGrid =
    var newGrid = this
    locsAndValues.foreach(locIsValue => newGrid = newGrid where locIsValue)
    newGrid
